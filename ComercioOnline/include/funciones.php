...
function mostrarFormulario($esNuevoUsuario, $btnEliminar, $form_action, $nick_readonly, $nick, $nombre, $apellidos, $telefono, $direccion, $tipo){
    ?>
        <form action="<?= $form_action ?>" method="post">
            ...
            <?php
                if($nick_readonly){
                    echo 'Nick:<br><input name="nick" value="' . $nick . '" readonly>';
                }else{
                    echo 'Nick:<br><input name="nick" value="' . $nick . '">';
                }
                if($esNuevoUsuario == false){
                    echo '<br><br>Antigua contraseña:<br><input name="antigua_contrasena" type="text">';
                }
            ?>
            <br><br>
            Contraseña:<br><input name="contrasena" type="text" minlength="6">
            <br><br>
            Confirmar contraseña:<br><input name="confirmar" type="text" minlength="6">
            <br><br>
            Nombre:<br><input name="nombre" value="<?= $nombre ?>">
            <br><br>
            Apellidos:<br><input name="apellidos" value="<?= $apellidos ?>">
            <br><br>
            Teléfono:<br><input name="telefono" value="<?= $telefono ?>">
            <br><br>
            Dirección:<br><input name="direccion" value="<?= $direccion ?>">
            <br><br>
            <?php
                ...
                if($btnEliminar == 'eliminar_usuario.php'){
                    ?>
                        ...
                        <button  class="boton" onclick="this.parentNode.action = '../web_ejecutores/eliminar_usuario.php';this.parentNode.submit()"><i class="fa fa-trash" aria-hidden="true"></i></button>

                    <?php
                }else{
                    ?>
                        <button class="boton" type="submit"><i class="fa fa-floppy-o" aria-hidden="true"></i></button>
                        <button class="boton"  onclick="this.parentNode.action = '../web_ejecutores/eliminarse_a_si_mismo.php';this.parentNode.submit()">Darme de baja</button>
                    <?php
                }
            ?>
        </form>
...
function mostrarFormularioSubirProductos($form_action, $btn_delete, $referencia_readonly, $referencia, $titulo, $descripcion, $ruta_imagen, $precio){
    ...
    <form action="<?= $form_action ?>" method="post"  enctype="multipart/form-data">
        ...
        if($ruta_imagen != ''){
            ...
            ?>
            Imágen:<br><input type="file" name="ruta_imagen" id="prueba"  multiple="multiple" value="../<?= $ruta_imagen ?>">
            <?php
        }
        ?>
        <br><br>
        ...
        if($referencia_readonly == true){
            echo 'Referencia:<br><input name="referencia" value="' . $referencia . '" readonly>';
        }else{
            echo 'Referencia:<br><input name="referencia" value="' . $referencia . '">';
        }
        ?>
        <br><br>
        Título:<br><input name="titulo" value="<?= $titulo ?>">
        <br><br>
        Descripción:<br><textarea name="descripcion" value=""><?= $descripcion ?></textarea>
        <br><br>

        Precio:<br><input type="number" name="precio" value="<?= $precio ?>">
        <br><br>
        <button class="boton" type="submit"><i class="fa fa-floppy-o" aria-hidden="true"></i></button>
        <?php
            if($btn_delete == true){
                ?>
                <button class="boton"  onclick="this.parentNode.action = '../web_ejecutores/eliminar_producto.php';this.parentNode.submit()"><i class="fa fa-trash" aria-hidden="true"></i></button>
                <?php
            }
        ?>
    </form>
...
function mostrarProductosEnGaleria(){
    echo '<br><br>';
...
function mostrarProductosEnCarrito($ruta_imagen, $titulo, $precio, $referencia){
    ...
    <div class="cont-producto-carrito">
        ...
        <form action="../web_ejecutores/eliminar_producto_de_carrito.php" method="post"><!--********************************  action y post  ***************************-->
...
function mostrarRecuadroListaPrecioTotalCarrito($nick){
    ...
    ?>
        <form class="cont-factura-carrito" action="../web_ejecutores/finalizar_compra_carrito.php" method="post">
...
function mostrarFactura($nick){/* https://codepen.io/uminily/pen/GrWPmw */
    ...
    $num_referencia = numeroUltimoRegistroHistoricos();
    $largo = count($num_referencia);
    for($i = $largo; $i < 5; $i++){/*Para hacer un poco mas chulo el numero de referencia*/
        $num_referencia = '0' . $num_referencia;
    }
        ...
        <div class="invoice">
            <div class="row">
                <div class="col-5">
                    ...
                    <p class="text-right"><strong>N° <?= $num_referencia ?></strong></p>
                </div>
            </div>
            <div class="row">
                <div class="col-7">
